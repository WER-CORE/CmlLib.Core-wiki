...
Ця бібліотека спрямована на підтримку встановлення всіх версій Forge, починаючи з 1.7.10. Версії до 1.7.10 також можна встановити, але це потребує додаткових дій. Інструкції для цього ви знайдете нижче на цій сторінці. Усі версії після 1.7.10 мають встановлюватися та працювати у звичайному режимі. Якщо щось не працює, будь ласка, створіть issue.

## Анотація

| Діапазон               | Підсумок                                      |
| --------------------- | --------------------------------------------- |
| 1.1 ~ 1.7.2           | ⚠️ Часткова підтримка                          |
| 1.7.10 ~ 1.21.4       | ✅ ПІДТРИМУЄТЬСЯ, протестовано                |
| після 1.21.4 ~ майбутні версії | ✅ ПІДТРИМУЄТЬСЯ, не протестовано, але має працювати |

## Усі результати тестування

Назва версії після іконки позначки означає версію Forge, що використовувалася під час тестування. Під час тестів застосовувалася або рекомендована, або найновіша версія Forge. Успішно протестована версія Minecraft має встановлюватися та працювати й з іншими версіями Forge.

Версії, не зазначені в цій таблиці, є непротестованими. Знову ж таки, версії після 1.7.10 мають встановлюватися та працювати, навіть якщо їх немає в таблиці.

| Версія    | Результат тестування                                |
| --------- | --------------------------------------------------- |
...
| 1.7.10_pre4 | ❌ не вдалося визначити версію гри                |
| 1.7.2     | ❌ краш гри                                         |
| 1.6.4     | ⚠️ 1.6.4-Forge9.11.1.1345, потрібні додаткові аргументи |
| 1.6.3     | ⚠️ 1.6.3-Forge9.11.0.878, потрібні додаткові аргументи  |
| 1.6.2     | ⚠️ 1.6.2-Forge9.10.1.871, потрібні додаткові аргументи  |
| 1.6.1     | ⚠️ Forge8.9.0.775, потрібні додаткові аргументи         |
| 1.5.2     | ⚠️ 1.5.2-Forge7.8.1.738, потрібні додаткові бібліотеки  |
| 1.5.1     | ⚠️ 1.5.1-Forge7.7.2.682, потрібні додаткові бібліотеки  |
| 1.5       | ⚠️ 1.5-Forge7.7.0.598, потрібні додаткові бібліотеки    |
| 1.4.7     | ⚠️ 1.4.7-Forge6.6.2.534, потрібні додаткові бібліотеки  |
| 1.4.6     | ⚠️ 1.4.6-Forge6.5.0.489, потрібні додаткові бібліотеки  |
| 1.4.5     | ⚠️ 1.4.5-Forge6.4.2.448, потрібні додаткові бібліотеки  |
| 1.4.4     | ⚠️ 1.4.4-Forge6.3.0.378, потрібні додаткові бібліотеки  |
| 1.4.3     | ⚠️ 1.4.3-Forge6.2.1.358, потрібні додаткові бібліотеки  |
| 1.4.2     | ⚠️ 1.4.2-Forge6.0.1.355, потрібні додаткові бібліотеки  |
| 1.4.1     | ⚠️ 1.4.1-Forge6.0.0.329, потрібні додаткові бібліотеки  |
| 1.3.2     | ⚠️ 1.3.2-Forge4.3.5.318, потрібні додаткові бібліотеки  |
| 1.2.5     | ✅ 1.2.5-Forge3.4.9.171                             |
| 1.2.4     | ⚠️ 1.2.4-Forge2.0.0.68, потрібен ModLoader          |
| 1.2.3     | ⚠️ 1.2.3-Forge1.4.1.64, потрібен ModLoader          |
| 1.1       | ⚠️ 1.1-Forge1.3.4.29, потрібен ModLoader            |

## Для 1.1 ~ 1.2.4

Старіші версії Forge не були автономними. Ці версії вимагають наявності [ModLoader](https://mcarchive.net/mods/modloader). Після встановлення цих версій Forge необхідно пропатчити ModLoader.

## Для 1.3.2 ~ 1.5.2

| Версія гри                                | Назва файлу                               | Контрольна сума SHA1                       |
| :---------------------------------------- | :---------------------------------------- | :----------------------------------------- |
...
Розмістіть ці файли у папці `<директорія_гри>/lib`.

## Для 1.6.1 ~ 1.6.4

Потребує додаткового аргументу JVM: `-Dfml.ignoreInvalidMinecraftCertificates=true`
